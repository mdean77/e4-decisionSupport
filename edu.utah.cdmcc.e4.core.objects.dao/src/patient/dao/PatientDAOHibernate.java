...
import core.dao.GenericHibernateDAO;
...
// TODO Need to fix the message dialogs in PatientDAOHibernate.  Will want to use injection
//		but not ready to implement this until I build the e4 model.
...
//	}
	...
	public boolean updatePatientValues(final Patient patient) {
		...
		System.out.println("Returned from validator with error length: "+validationMessages.length);
		if (validationMessages.length == 0) {
			System.out.println("Validation errors zero so about to make persistent in updatePatientValues");
